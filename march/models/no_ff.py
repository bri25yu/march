...
class NoFFEncoder(TransformerComponentBase):
    ...
    def __init__(self, config: TransformerConfig) -> None:
        ...
        self.layernorms = ModuleList(
            [LayerNorm(config) for _ in range(config.num_layers // 2 + 1)]
        )
        self.self_attention_layers: List[AttentionBase] = ModuleList(
            [
                self.ATTENTION_CLS(
                    config, is_cross_attention=False, has_relative_attention_bias=i == 0
                )
    ...
    def forward(
        ...
        for i in range(config.num_layers // 2):
            ...
            normed_input_embeds: SequenceInputEmbeds = self_attention_layernorm(
                input_embeds
            )
            self_attention_output: AttentionOutput = self_attention_layer(
                normed_input_embeds, attention_mask, position_bias
            )
            input_embeds: SequenceInputEmbeds = input_embeds + dropout(
                self_attention_output.input_embeds,
...
class NoFFDecoder(TransformerComponentBase):
    ...
    def __init__(self, config: TransformerConfig) -> None:
        ...
        self.layernorms = ModuleList(
            [LayerNorm(config) for _ in range(config.num_layers + 1)]
        )
        self.self_attention_layers: List[AttentionBase] = ModuleList(
            [
                self.ATTENTION_CLS(
        ...
        self.cross_attention_layers: List[AttentionBase] = ModuleList(
            [
                self.ATTENTION_CLS(config, is_cross_attention=True)
    ...
    def forward(
        ...
        for i in range(config.num_layers // 2):
            ...
            normed_input_embeds: SequenceInputEmbeds = cross_attention_layernorm(
                input_embeds
            )
            cross_attention_output: AttentionOutput = cross_attention_layer(
                normed_input_embeds,
                encoder_attention_mask,
                encoder_hidden_state=encoder_hidden_state,
            )
            input_embeds: SequenceInputEmbeds = input_embeds + dropout(
                cross_attention_output.input_embeds,
                p=config.dropout_prob,
                training=self.training,
            )
