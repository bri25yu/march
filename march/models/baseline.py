...
class DecoderBase(TransformerComponentBase):
    ...
    def __init__(self, config: TransformerConfig) -> None:
        ...
        crossattn_kwargs = dict(
            is_cross_attention=True, is_decoder=True, has_relative_attention_bias=False
        )
    ...
    def forward(
        ...
        for selfattn_ln, selfattn, crossattn_ln, crossattn, ff_ln, ff in self.layers:
            ...
            cross_attention_output: AttentionOutput = crossattn(
                crossattn_ln(input_embeds),
                encoder_attention_mask,
                encoder_hidden_state=encoder_hidden_state,
            )
            input_embeds = self.apply_residual(
                input_embeds, cross_attention_output.input_embeds
            )
