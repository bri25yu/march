...
def update_L_dimension(
    ...
    expanded_mask_keep = (
        expanded_mask_keep[:, :, None, :] & expanded_mask_keep[:, :, :, None]
    )
    assert expanded_mask_keep.size() == (N, H, L, L)
    position_bias = masked_select(position_bias, expanded_mask_keep).reshape(
        N, H, L_out, L_out
    )
...
def calculate_mask_drop(
    attention_probs: FloatTensor, L_drop: int
) -> Tuple[BoolTensor, LongTensor, int]:
    """
    ...
    bottomk_indices = attention_probs_for_mask.topk(
        k=effective_L_drop, dim=1, largest=False
    ).indices
...
def log_fcabs(
    ...
    """
    if dropped_indices_by_layer is None:
        return None

    N, L_out = input_ids.size()

    dropped_ids_by_layer = []  # (N_L, N, L_drop)
    for dropped_indices, mask_drop, effective_L_drop in zip(
        dropped_indices_by_layer, mask_drop_by_layer, effective_L_drop_by_layer
    ):
...
class FCABSAttention(BaselineAttention):
    def forward(
        ...
        key_value_state = (
            encoder_hidden_state if self.is_cross_attention else input_embeds
        )
        query, key, value = list(
            map(
                ...
                [
                    ...
                    self.w_k(key_value_state),
                    self.w_v(key_value_state),
                ],
            )
        )
        ...
        mask_drop, bottomk_indices, effective_L_drop = calculate_mask_drop(
            attention_probs, config.L_drop
        )  # (N, L)
...
class FCABSEncoder(BaselineEncoder):
    ...
    def forward(
        ...
        for selfattn_ln, selfattn, ff_ln, ff in self.layers:
            ...
            self_attention_output: FCABSAttentionOutput = selfattn(
            ...
            input_embeds, attention_mask, position_bias = update_L_dimension(
                ...
                attention_mask,
                position_bias,
                self_attention_output.mask_drop,
                self_attention_output.effective_L_drop,
            )
            if return_fcabs:
...
class FCABSTransformer(BaselineTransformer):
    ...
    def forward(
        ...
        encoder_outputs: FCABSEncoderOutput = self.encoder(
            input_embeds, attention_mask, return_fcabs=return_fcabs
        )
        encoder_hidden_state = encoder_outputs.input_embeds

        decoder_input_embeds: SequenceInputEmbeds = embedding(
            decoder_input_ids, self.embedding.weight
        )
        ...
        return FCABSTransformerOutput(
            loss=loss, logits=lm_logits, dropped_ids=dropped_ids
        )
