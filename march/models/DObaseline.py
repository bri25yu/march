...
class DOTransformerBase(TransformerComponentBase):
    ...
    def forward(
        ...
        decoder_input_embeds: SequenceInputEmbeds = embedding(input_ids, self.embedding.weight)
        decoder_outputs: AttentionOutput = self.decoder(decoder_input_embeds, attention_mask)
...
class DOBaselineAttention(AttentionBase):
    ...
    def forward(
        ...
        query, key, value = list(map(
            ...
            [self.w_q(input_embeds), self.w_k(input_embeds), self.w_v(input_embeds)]
        ))
