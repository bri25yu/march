...
from march.models.baseline import BaselineFeedforward

"""
Decoder only implementation of the Transformer architecture.
Based off the GPT2 implementation in https://github.com/huggingface/transformers/blob/main/src/transformers/models/gpt2/modeling_gpt2.py
"""
...
__all__ = [
    ...
    "DOBaselineAttention",
    "DOBaselineDecoder",
    "DOBaselineTransformer",
]


class DODecoderBase(TransformerComponentBase):
    @property
    @abstractmethod
        ...
        for i in range(config.num_layers):
            self.layers.append(
                ModuleList(
                    [
                        LayerNorm(config),
                        attn_cls(config, has_relative_attention_bias=i == 0),
                        LayerNorm(config),
                        ff_cls(config),
                    ]
                )
    ...
    def forward(
        self, input_embeds: SequenceInputEmbeds, attention_mask: SequenceInputIds
    ) -> AttentionOutput:
        ...
        for selfattn_ln, selfattn, ff_ln, ff in self.layers:
            self_attention_output: AttentionOutput = selfattn(
                selfattn_ln(input_embeds), attention_mask, position_bias
            )
            input_embeds = self.apply_residual(
                input_embeds, self_attention_output.input_embeds
            )
...
class DOTransformerBase(TransformerComponentBase):
    @property
    @abstractmethod
    def DECODER_CLS(self) -> Type[DODecoderBase]:
        pass
    ...
    def __init__(self, config: TransformerConfig) -> None:
        ...
        self.embedding: TensorType["D", "V"] = Linear(
    ...
    def forward(
        ...
        decoder_input_embeds: SequenceInputEmbeds = embedding(
            input_ids, self.embedding.weight
        )
        decoder_outputs: AttentionOutput = self.decoder(
            decoder_input_embeds, attention_mask
        )
...
class DOBaselineAttention(AttentionBase):
    def __init__(
        self, config: TransformerConfig, has_relative_attention_bias: bool = False
    ) -> None:
        ...
        self.w_q = Linear(
            config.dim_model, config.num_heads * config.dim_qkv, bias=False
        )
        self.w_k = Linear(
            config.dim_model, config.num_heads * config.dim_qkv, bias=False
        )
        ...
        self.w_o = Linear(
            config.num_heads * config.dim_qkv, config.dim_model, bias=False
        )
    ...
    def init_weights(self) -> None:
        ...
        self.w_q.weight.data.normal_(
            mean=0.0, std=(config.dim_model * config.dim_qkv) ** -0.5
        )
        ...
        self.w_o.weight.data.normal_(
            mean=0.0, std=(config.num_heads * config.dim_qkv) ** -0.5
        )

        if self.has_relative_attention_bias:
            self.relative_attention_bias.weight.data.normal_(
                mean=0.0, std=config.dim_model**-0.5
            )
    ...
    def forward(
        ...
        attention_probs: MultiHeadedAttention = softmax(
            attention_logits.to(float32), dim=3
    ...
    def _relative_position_bucket(
        ...
        relative_position_if_large = min(
            ...
            full_like(relative_position_if_large, num_buckets - 1),
        )
    ...
    def compute_bias(
        ...
        if not has_relative_attention_bias:
            position_bias = zeros(
                (1, config.num_heads, query_length, key_length),
                device=device,
                dtype=dtype,
            )
...
class DOBaselineDecoder(DODecoderBase):
    ATTENTION_CLS = DOBaselineAttention
    FEEDFORWARD_CLS = BaselineFeedforward


class DOBaselineTransformer(DOTransformerBase):
    DECODER_CLS = DOBaselineDecoder
